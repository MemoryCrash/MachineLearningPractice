...
class DeepQNetwork:
    ...
    def __init__(
        ...
        if output_graph:
            ...
            tf.summary.FileWriter("logs_bird/", self.sess.graph)
    ...
    def net_saver(self, step):
        if step % 50000 == 0:
            self.saver.save(self.sess, 'saved_networks_bird/bird-dqn', global_step=step)

    def net_restore(self):
        checkpoint = tf.train.get_checkpoint_state("saved_networks_bird")
